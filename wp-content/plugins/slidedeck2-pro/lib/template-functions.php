...
/**
 * Get the icon for the content's taxonomy type
 * 
 * Looks up the icon associated with the SlideDeck's primary (first) taxonomy group and
 * echo's out the IMG tag (if $echo is set to boolean(true)) or returns the image's source.
 * 
 * @param array $slidedeck The SlideDeck object
 * @param boolean $echo Echo the IMG tag with the SlideDeck's icon
 * 
 * @global $SlideDeckPlugin
 * 
 * @return string
 */
if( !function_exists( 'slidedeck2_icon' ) ) {
	function slidedeck2_icon( $slidedeck, $echo = true ) {
	    global $SlideDeckPlugin;
	    
	    if( empty( $slidedeck['type'] ) || empty( $slidedeck['source'] ) )
	        return "";
	    
	    $deck_classname = $SlideDeckPlugin->get_classname( $slidedeck['type'] );
	    $deck = $SlideDeckPlugin->decks[$deck_classname];
	    $source = $deck->sources[$slidedeck['source']];
	    $image_src = SLIDEDECK2_URLPATH . $source['icon'];
	    
	    if( $echo == true ) {
	        echo '<img class="icon ' . strtolower( $deck_classname ) . ' ' . $source['name'] . '" src="' . $image_src . '" alt="' . $source['label'] . '" />';
	    }
 ...
 * @global $SlideDeckPlugin
 * 
 * @uses SlideDeckPlugin::get_source_primary_taxonomy()
 * 
 * @return string
 */
if( !function_exists( 'slidedeck2_taxonomy_icon' ) ) {
	function slidedeck2_taxonomy_icon( $slidedeck, $echo = true ) {
	    global $SlideDeckPlugin;
	    ...
	    $taxonomy = $SlideDeckPlugin->taxonomies[$source_primary_taxonomy];
	    $image_src = SLIDEDECK2_URLPATH . $taxonomy['icon'];
	    
	    if( $echo == true ) {
	        echo '<img src="' . $image_src . '" alt="' . $taxonomy['label'] . '" />';
	    }
 ...
 * @param string $time_from_now time in seconds from now when the cache should expire
 * 
 * @uses set_transient()
 * 
 * @return boolean
...
if( !function_exists( 'slidedeck2_cache_write' ) ) {
	function slidedeck2_cache_write( $name = "", $content = "", $time_from_now = 1800 ) {
	    ...
	    switch( $type ) {
	        ...
	        case "radio":
	            ...
	            foreach( $values as $radio_value => $radio_text ){
	                $html .= '<label for="' . $id . '-' . $radio_value . '" class="label">' . $required_str . __( $radio_text, 'slidedeck' );
	                $html .= $tooltip_str;
	                $html .= $thumbnail_str;
	                $html .= '<input id="' . $id . '-' . $radio_value . '" type="radio" name="' . $name . '" value="' . $radio_value . '"' . ( $radio_value == $value ? ' checked="checked"' : '' );
		...
		<div id="taxonomy-<?php echo $taxonomy; ?>" class="categorydiv">
			...
			<div id="<?php echo $taxonomy; ?>-all" class="tabs-panel">
				...
				<ul id="<?php echo $taxonomy; ?>checklist" class="list:<?php echo $taxonomy?> categorychecklist form-no-clear">
					<?php wp_terms_checklist($post->ID, array( 'taxonomy' => $taxonomy, 'selected_cats' => $args['selected_cats'], 'popular_cats' => $popular_ids ) ) ?>
				</ul>
			</div>
		</div>
		<?php
...
if( !function_exists( 'slidedeck2_stip_tags_and_truncate_text' ) ) {
	function slidedeck2_stip_tags_and_truncate_text( $str, $length = 55, $suffix = "&hellip;" ) {
	    $truncated = trim( mb_substr( strip_tags( $str ), 0, $length ) );
