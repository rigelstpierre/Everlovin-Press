<?php
class SlideDeck_Social extends SlideDeck {
    ...
    var $default_slide_count = 10;
    // Default options for this Deck type
    var $default_options = array(
        'feedCacheDuration' => 300 // seconds
    );
    var $twitter_fetch_user_cache = 172800;
    ...
    var $sources = array(
        'twitter' => array(
            'name' => 'twitter',
            'label' => "Twitter Timeline/Search",
            'icon' => '/decks/social/images/twitter-icon.png',
            'chicklet' => '/decks/social/images/twitter-chicklet.png',
            'taxonomies' => array( 'social' ),
            'default_lens' => "twitter"
        )
    );
     ...
     * Twitter Source
     */
    function content_source_twitter( $slidedeck, $form_action ) {
        // Fail silently if the SlideDeck is not this type or source
        if( $slidedeck['type'] != $this->type || $slidedeck['source'] != 'twitter' ) {
            return false;
        }
        ...
        include( dirname( __FILE__ ) . '/views/source/_twitter.php' );
    }
     ...
     * @return array
     */
    function get_twitter_slides( $slidedeck ) {
        ...
        // Create a cache key
        $cache_key = $slidedeck_id . $feed_url . $slidedeck['options']['twitter_scrape_images'] . $slidedeck['options']['useGeolocationImage'] . $slidedeck['options']['feedCacheDuration'] . $this->type;
        ...
        if( !$twitter_posts ){
                            ...
                            if( is_object( $result ) ) {
                                if( isset( $result->retweeted_status ) ){
                                    ...
                                    $twitter_posts[ $index ] = array(
                                        ...
                                        'created_at' => $result->retweeted_status->created_at,
            ...
            // Write the cache
            slidedeck2_cache_write( $cache_key, $twitter_posts, $slidedeck['options']['feedCacheDuration'] );
        }
    ...
    function slidedeck_get_slides( $slides, $slidedeck ) {
        ...
        switch( $slidedeck['source'] ) {
            case "twitter":
                $slides_nodes = $this->get_twitter_slides( $slidedeck );
            break;
        }
    ...
    function slidedeck_options_model( $options_model, $slidedeck ){
        if( $slidedeck['type'] == $this->type  ) {
            $hide_excerpt_sources = array('twitter');
            if( in_array( $slidedeck['source'] , $hide_excerpt_sources ) ){
                $options_model['Display']['hide-excerpt'] = array(
                    'type' => 'hidden',
                    'value' => true,
                    'data' => 'boolean'
                );
			...
			if( $slidedeck['source'] == "twitter" ) {
				$options_model['Setup']['total_slides']['label'] = "Number of Tweets";
				$options_model['Content']['validateImages']['type'] = "hidden";
				$options_model['Content']['date-format']['type'] = "hidden";
                $options_model['Content']['useGeolocationImage']['type'] = 'checkbox';
