(function($){
    SlideDeckSkin['o-town'] = function(slidedeck){
        ...
        var elems = {};
            ...
            // The slide-image element in each dd
            elems.verticalSlideImages = elems.verticalSlides.find('div.slide-image');
        ...
        if( !isThumbnailNav ){
            ...
            if( $.browser.msie ){
                if( $.browser.version <= 8.0 ){
                    elems.verticalSlideImages.each(function(ind){
                        if( $(elems.verticalSlideImages[ind]).css('background-image') != 'none' ){
                            var imgurl = $(elems.verticalSlideImages[ind]).css('background-image').match( /url\([\"\'](.*)[\"\']\)/ )[1];
                            $(elems.verticalSlideImages[ind]).css({
                                background: 'none'
                            });
                            elems.verticalSlideImages[ind].style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='" + imgurl + "', sizingMethod='scale')";
                        };
                    });
                }
            }
        ...
        elems.slidedeck.slidedeck().loaded(function(){
            ...
            if( isThumbnailNav ){
                ...
                elems.verticalSlideImages.each(function(ind){
                    ...
                    thumbSrc[ind] = $(elems.verticalSlides[ind]).attr('data-thumbnail-src');
                    
                    // Only for IE - detect background image url and update style for DD element
                    if( $.browser.msie && ($.browser.version <= 8.0)  ){
                        if( $(elems.verticalSlideImages[ind]).css('background-image') != 'none' ){
                            var imgurl = $(elems.verticalSlideImages[ind]).css('background-image').match( /url\([\"\'](.*)[\"\']\)/ )[1];
                            $(navigation.verticalNavLinksInner[ind]).css({
                                background: 'none'
                            });
                            if( thumbSrc[ind] ){
                        ...
                        if( thumbSrc[ind] ){
                            ...
                            thumbSrc[ind] = $(elems.verticalSlideImages[ind]).css('background-image');
                        }
                        if( $(elems.verticalSlideImages[ind]).css('background-image') != 'none' ){
                            $(navigation.verticalNavLinksInner[ind]).css({
            ...
            verticalDeck.options.before = function(deck){
                ...
                if( isAutoPlay && !$(slidedeck).slidedeck().pauseAutoPlay ){
                    ...
                    var nextVertSlideIndex = deck.current + 1;
