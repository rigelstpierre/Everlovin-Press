...
(function($){
        ...
        insertSlideDecks: function(){
            ...
            for(var d in datas){
                ...
                if(data.name == "slidedecks[]"){
                    shortcodes.push("[SlideDeck2 id=" + data.value + "]");
                }
            }
    ...
    SlideDeckPlugin.LensEditor = {
        ...
        initialize: function(){
            ...
            if(this.textarea.length){
                ...
                var filename = $('#slidedeck-lens-filename').val();
                // Look at the file requested to determine syntax highlighting
                var editorMode = filename.match(/\.css$/) ? "css" : (filename.match(/\.js$/) ? "javascript" : "text/x-php");
                // Look at the readonly property of the textarea to determine its writable status
                var isReadOnly = (typeof(this.textarea.attr('readonly')) != 'undefined');
                
                this.editor = CodeMirror.fromTextArea(this.textarea[0], {
                    lineNumbers: true,
                    mode: editorMode,
                    theme: isReadOnly ? "disabled" : "slidedeck",
                    readOnly: isReadOnly,
                    indentUnit: 4,
                    tabSize: 4,
                    lineWrapping: true,
    ...
    SlideDeckPlugin.LensManager = {
        ...
        select: function($elem){
            ...
            this.elems.lenses.removeClass('selected');
        ...
        initialize: function(){
            ...
            this.elems.lensesSection = $('#slidedeck-section-lenses');
            ...
            this.elems.lensesSection.delegate('input[type="radio"]', 'click', function(){
    ...
    SlideDeckPlugin.OptionsNav = {
        ...
        deckLoaded: function(slidedeck){
            ...
            this.elems.navigation = $('#slidedeck-options-groups .verticalSlideNav');
            
            this.elems.navigation.delegate('a', 'click', function(event){
                event.preventDefault();
            ...
            switch(type){
                case "slider":
                    var $option = $('#options-' + updateObj.option);
                        $option.val(Math.min(parseInt($option.val()), parseInt(value)));
                    ...
                    if($slider.length){
                        ...
                        $slider.slider('value', parseInt($option.val()));
                    }
                break;
            }
        },
        ...
        initialize: function(){
            ...
            this.elems.optionsGroupsLists.css('min-height', this.elems.optionsGroupsLists.length * 47);
        ...
        resize: function(){
            ...
            this.elems.optionsGroups.stop().animate({
                height: this.elems.optionsGroupsLists.eq(this.elems.slidedeck.slidedeck().vertical().current).height() + "px"
            }, 500);
        }
    };
    ...
    SlideDeckPlugin.SourceManager = {
        ...
        select: function(el){
            ...
            if(action == "create"){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data != "false"){
                            $('#slidedeck-content-source').html(data);
                        }
                    }
                });
            }
        ...
        toggle: function(elem){
            var self = this;
            this.elems.toggles.removeClass('selected');
            $(elem).addClass('selected');
            
            var view = elem.href.split('#')[1];
            
            var transitionSpeed = 300;
            
            $('#no-decks-placeholder .prompt').hide();
            if(view == "decks"){
                this.toggleSlideDeck.goTo(1);
                $('#no-decks-placeholder .prompt.categories').show();
            } else if(view == "sources") {
                this.toggleSlideDeck.goTo(2);
                $('#no-decks-placeholder .prompt.sources').show();
            }
            
            $.get(ajaxurl + "?action=slidedeck_change_source_view&view=" + view);
        },
        
        initialize: function(){
            var self = this;
            
            this.elems.decks = $('#slidedeck-types');
            
            // Fail silently if there are no deck types to choose from
            if(this.elems.decks.length < 1){
                return false;
            }
            
            this.elems.viewTaxonomy = this.elems.decks.find('.decks');
            this.elems.viewSources = this.elems.decks.find('.sources');
            this.elems.toggles = this.elems.decks.find('.toggle');
            this.elems.toggleSlideDeck = $('#create-buttons-slidedeck');
            this.elems.modalLinks = $('a.slidedeck-source-modal');
            
            this.modal = new SimpleModal({
                context: "source",
                onComplete: function(modal){
                    modal.elems.modal.delegate('input[type="radio"]', 'click', function(){
            ...
            this.elems.modalLinks.bind('click', function(event){
                event.preventDefault();
                self.open(this.href);
            });
            
            this.elems.toggles.bind('click', function(event){
                event.preventDefault();
                self.toggle(this);
            });
            
            var selected = this.elems.toggles.filter('.selected');
            if(selected.length){
                selected = selected.attr('href').split('#')[1] == "decks" ? 1 : 2;
            } else {
                selected = 1;
            }
            
            this.toggleSlideDeck = this.elems.toggleSlideDeck.slidedeck({
                scroll: false,
                keys: false,
                hideSpines: true,
                start: selected
    ...
    SlideDeckPlugin.CoversEditor = {
        ...
        onComplete: function(modal){
            ...
            this.elems.modal.find('#show_curator').bind('click', function(){
                if(this.checked)
            ...
            // Peek preview
            this.elems.modal.find('#peek').bind('click', function(){
                if(this.checked)
    ...
    $(document).ready(function(){
        ...
        $('#slidedeck-table').delegate('.slidedeck-preview-link', 'click', function(event){
            ...
            var $this = $.data(this, '$this'),
                ...
                $td = $.data(this, '$td'),
                iframeSrc = this.href,
                outer_width = parseInt(this.href.match(/\&outer_width=(\d+)/)[1]);
                outer_height = parseInt(this.href.match(/\&outer_height=(\d+)/)[1]);
        ...
        $('#slidedeck-update-form').delegate('.configure-source', 'click', function(event){
            event.preventDefault();
            $('#slidedeck-content-source').toggleClass('hidden');
        });
        ...
        if( $('#slidedeck-content-source .cache-slider').length ){
            ...
            var currentValue = $.inArray( parseInt( $('#options-feedCacheDuration').val() ), minutesValues );
            ...
            $('#slidedeck-content-source .cache-slider').slider({
                ...
                slide: function( event, ui ) {
                    $( "#slidedeck-content-source .cache-slider-value" ).html( humanValues[ ui.value ] );
                    $('#options-feedCacheDuration').val( minutesValues[ ui.value ] );
                },
                create: function( event, ui ){
                    // Assign the current value (on page load) to the label. 
                    $( "#slidedeck-content-source .cache-slider-value" ).html( humanValues[ currentValue ] );
                }
            });
        }
        ...
        $('#slidedeck-title-wrapper-edit-view').delegate('a', 'click', function(event){
            event.preventDefault();
            
            var $wrapper = $.data(this, '$wrapper'),
                $title = $.data(this, '$title'),
                $titleInput = $.data(this, '$titleInput'),
                $editView = $.data(this, '$editView');
            
            if(!$wrapper){
                $wrapper = $('#slidedeck-title-wrapper');
                $.data(this, '$wrapper', $wrapper);
            }
            ...
            if(!$titleInput){
                $titleInput = $('#slidedeck_title');
                $.data(this, '$titleInput', $titleInput);
            }
            
            if(!$editView){
                $editView = $('#slidedeck-title-wrapper-edit-view');
                $.data(this, '$editView', $editView);
            }
            
            $titleInput.width($title.width()).fadeIn().focus();
            $editView.fadeOut();
        });
        $('#slidedeck_title').bind('blur', function(event){
            var $formAction = $.data(this, '$formAction'),
                $editView = $.data(this, '$editView');
            
            if(!$formAction){
                $formAction = $('#form_action');
                $.data(this, '$formAction', $formAction);
            }
            ...
            $wrapper.find('h1 span').text(this.value);
        });
