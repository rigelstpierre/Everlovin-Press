...
class SlideDeckCovers {
    ...
    function __construct() {
        ...
        $this->fonts = $slidedeck_fonts;
        
        foreach( $slidedeck_fonts as $font => $properties ) {
            $this->options_model['title_font']['values'][$font] = $properties['label'];
        }
    ...
    function get( $slidedeck_id ) {
        ...
        $cache_key = md5( __CLASS__ . __METHOD__ . $parent_slidedeck_id );
        
        $cover = wp_cache_get( $cache_key );
    ...
    function render( $slidedeck_id, $front_back = 'front' ) {
        ...
        $curator_name = !empty( $slidedeck_author->user_nicename ) ? $slidedeck_author->user_nicename : $slidedeck_author->user_login;
