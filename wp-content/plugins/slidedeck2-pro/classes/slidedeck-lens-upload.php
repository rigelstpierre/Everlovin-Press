...
class SlideDeck_Lens_Upload extends WP_Upgrader {
	...
	function install( $package ) {
		...
		$options = array(
			...
			'clear_working' => true,
			'hook_extra' => array(
                'lens_dirname' => $this->lens->options['lens_dirname']
            )
		);
    ...
    function slidedeck_lens_destination( $destination, $source, $hook_extra ) {
        ...
        if( isset( $hook_extra['lens_dirname'] ) ) {
            $lens_dirname = $hook_extra['lens_dirname'];
        }
        // Otherwise, figure it out from the source's file name
        else {
            $lens_dirname = preg_replace( "/\.([a-zA-Z0-9]+)$/", "", basename( $source ) );
            $copy_inc = $SlideDeckPlugin->Lens->copy_inc( $lens_dirname );
            if( $copy_inc > 0 ) {
