...
class SlideDeckLens {
		...
		if( $original_lens ) {
			...
			foreach( (array) $lens_files as $lens_file ) {
				...
				switch( $file_ext ) {
                    ...
                    case "php":
                        $old_classname = $SlideDeckPlugin->get_classname( $original_lens['slug'] );
                        $new_classname = $SlideDeckPlugin->get_classname( $new_slug );
    ...
    function get_meta( $filename ) {
        ...
        if( $lens == false ) {
            ...
            $lens_url = site_url( str_replace( str_replace( "\\", "/", ABSPATH ), "", str_replace( "\\", "/", $lens_folder ) ) );
			...
			foreach( $SlideDeckPlugin->taxonomies as $taxonomy => $tax_properties ) {
				$template_file = $lens_folder . '/template.' . $taxonomy . '.thtml';
				if( file_exists( $template_file ) ) {
					$lens['templates'][$taxonomy] = $template_file;
				}
			}
    ...
    function process_template( $nodes = array(), $slidedeck ) {
        ...
        // Check for taxonomy type template override
		$template = $lens['templates']['default'];
		$source_primary_taxonomy = $SlideDeckPlugin->get_source_primary_taxonomy( $slidedeck['source'] );
		if( isset( $lens['templates'][$source_primary_taxonomy] ) ) {
			$template = $lens['templates'][$source_primary_taxonomy];
		}
        
        // Check for source type template override
		if( isset( $lens['templates'][$slidedeck['source']] ) ) {
			$template = $lens['templates'][$slidedeck['source']];
		}
